...
// Get unreminded error logs (status_reminder is NULL or empty only) - ORDER BY DESC for latest first
const getUnremindedErrorLogs = async () => {
  try {
    const result = await pool.query(
      "SELECT * FROM error_msg_log WHERE (status_reminder IS NULL OR status_reminder = '') ORDER BY created_date DESC"
    );
    return result.rows;
  } catch (err) {
